 ...
 * Permite al usuario visualizar su carrito y confirmar la compra
 */
 ...
 * - Mostrar carrito activo con items
 * - Validar estado (no vacío, stock disponible)
 * - Permitir confirmación que genera orden
 * - Navegar a orden success
 */
const CartConfirmationPage: React.FC = () => {
	const navigate = useNavigate();
	const { user, isAuthenticated } = useAuth();
	const { items: cartItems = [], total = 0, totalQuantity = 0, updateQuantity, removeItem } = useCart();
	const { isLoading, error, data, confirmCart } = useCartConfirmation();
	const [localError, setLocalError] = useState<string | null>(null);
	const cartId = ''; // TODO: Get from cart context
	...
	// Navegar a página de éxito cuando se crea la orden
	useEffect(() => {
		if (data?.orderId) {
	...
	const handleConfirmClick = async () => {
		...
		// Validación 1: Carrito no está vacío
		if (cartItems.length === 0) {
			setLocalError('El carrito está vacío');
			return;
		}

		// Validación 2: Validar stock disponible
		const stockIssue = cartItems.find((item: CartItem) => item.quantity > item.stock);
		if (stockIssue) {
			setLocalError(
				`Stock insuficiente para ${stockIssue.name}. Disponible: ${stockIssue.stock}`
			);
			return;
		}

		// Validación 3: Tener cartId y userId
		if (!cartId || !user?.id) {
			setLocalError('Error: datos de carrito o usuario no disponibles');
			return;
		}

		// Llamar a confirmación
		try {
			await confirmCart(
				cartId,
				user.id,
			...
			setLocalError('Error al procesar la solicitud. Por favor, intenta de nuevo.');
				...
				<>
								...
								<li key={item.id} className="border-b pb-4 last:border-b-0">
									...
									{item.quantity > item.stock && (
										<div className="mt-2 bg-red-100 border border-red-400 text-red-700 p-2 rounded text-sm">
											⚠️ Stock insuficiente: {item.stock} disponible(s), solicitarte(s): {item.quantity}
										</div>
									)}
								</li>
							))}
							...
							{error?.details && (
								<p className="text-sm mt-2 text-red-600">
									...
									{error.details.available !== undefined &&
										`Disponible: ${error.details.available} | `}
					...
					<div className="flex justify-between gap-4">
						...
						className={`flex-1 px-8 py-3 rounded font-semibold transition flex items-center justify-center gap-2 ${
							...
							title={isConfirmDisabled ? 'El carrito está vacío o no hay stock disponible' : ''}
						>
							{isLoading ? (
								<>
